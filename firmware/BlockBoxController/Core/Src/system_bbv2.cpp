...
void BlockBoxV2System::Init() {
  ...
  this->amp_if.RegisterCallback([&](ModuleInterface&, uint32_t event) {
    switch (event) {
  ...
  this->dap_if.ResetModule([&](bool success) {
    ...
    if (success) {
      ...
      this->dap_if.SetVolumeGains({ 0.0f, 0.0f }, [&](bool success) {
  ...
  this->amp_if.InitModule([&](bool success) {
    DEBUG_PRINTF("PowerAmp reset/init complete, success %u\n", success);
    if (success) {
      this->amp_if.monitor_measurements = true;
      this->amp_if.SetManualShutdownActive(false, [&](bool success) {
        DEBUG_PRINTF("PowerAmp disabled manual shutdown, success %u\n", success);
        if (success) {
          this->amp_if.SetPVDDTargetVoltage(25.0f, [&](bool success) {
