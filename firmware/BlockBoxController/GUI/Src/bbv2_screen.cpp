...
void BlockBoxV2Screen::Init() {
  //TODO register event handler (with battery, charger, and all input-related modules) to force list re-build
}
...
void BlockBoxV2Screen::BuildScreenContent() {
  ...
  if (display_battery_soc) {
    ...
    //themed (full) section
    this->driver.CmdColorRGB(theme_color);
  ...
  if (this->status_text_override == NULL) {
    ...
    //overridden status text
    this->driver.CmdText(4, 3, 26, 0, this->status_text_override);
  }
