...
HAL_StatusTypeDef HAL_SPDIFRX_DMAStop(SPDIFRX_HandleTypeDef *hspdif)
{
  ...
  hspdif->Instance->CR &= (uint32_t)(~SPDIFRX_CR_RXDMAEN);
  hspdif->Instance->CR &= (uint32_t)(~SPDIFRX_CR_CBDMAEN);

  /* Disable the SPDIFRX DMA channel */
  if (hspdif->hdmaDrRx != NULL)
  {
    /* Disable the SPDIFRX data DMA Stream/Channel */
    if (HAL_OK != HAL_DMA_Abort(hspdif->hdmaDrRx))
    {
      SET_BIT(hspdif->ErrorCode, HAL_SPDIFRX_ERROR_DMA);
      errorcode = HAL_ERROR;
    }
  }
  if (hspdif->hdmaCsRx != NULL)
  {
    /* Disable the SPDIFRX control DMA Stream/Channel */
    if (HAL_OK != HAL_DMA_Abort(hspdif->hdmaCsRx))
    {
      SET_BIT(hspdif->ErrorCode, HAL_SPDIFRX_ERROR_DMA);
      errorcode = HAL_ERROR;
    }
...
static void SPDIFRX_DMAError(DMA_HandleTypeDef *hdma)
{
  ...
  hspdif->Instance->CR &= (uint32_t)(~(SPDIFRX_CR_RXDMAEN | SPDIFRX_CR_CBDMAEN));
  hspdif->RxXferCount = 0;
