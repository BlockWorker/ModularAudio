...
void _ADC_ProcessCallback(uint8_t index, uint8_t half) {
...
#ifdef P_ADC_FOUR_CHANNEL
  //select first and second channel based on index
  q15_t* raw_current_1 = index == 0 ? raw_current_A : raw_current_C;
  q15_t* raw_voltage_1 = index == 0 ? raw_voltage_A : raw_voltage_C;
  q15_t* raw_power_1 = index == 0 ? raw_power_A : raw_power_C;
  q15_t* raw_current_2 = index == 0 ? raw_current_B : raw_current_D;
  q15_t* raw_voltage_2 = index == 0 ? raw_voltage_B : raw_voltage_D;
  q15_t* raw_power_2 = index == 0 ? raw_power_B : raw_power_D;
  //extract currents from matrices
  arm_mat_vec_mult_q15(current_matrix, processing_vector_AC, raw_current_1);
  arm_mat_vec_mult_q15(current_matrix, processing_vector_BD, raw_current_2);
  //extract voltages from matrices
  arm_mat_vec_mult_q15(voltage_matrix, processing_vector_AC, raw_voltage_1);
  arm_mat_vec_mult_q15(voltage_matrix, processing_vector_BD, raw_voltage_2);
  //calculate instantaneous power
  arm_mult_q15(raw_voltage_1, raw_current_1, raw_power_1, P_ADC_SAMPLE_BATCH_SIZE);
  ...
  q15_t raw_results[6]; //Irms1, Vrms1, Pavg1, Irms2, Vrms2, Pavg2
  arm_rms_q15(raw_current_1, P_ADC_SAMPLE_BATCH_SIZE, raw_results);
  arm_rms_q15(raw_voltage_1, P_ADC_SAMPLE_BATCH_SIZE, raw_results + 1);
  arm_mean_q15(raw_power_1, P_ADC_SAMPLE_BATCH_SIZE, raw_results + 2);
  arm_rms_q15(raw_current_2, P_ADC_SAMPLE_BATCH_SIZE, raw_results + 3);
  arm_rms_q15(raw_voltage_2, P_ADC_SAMPLE_BATCH_SIZE, raw_results + 4);
  arm_mean_q15(raw_power_2, P_ADC_SAMPLE_BATCH_SIZE, raw_results + 5);
  //convert to floats and save
  float f_results[6];
  arm_q15_to_float(raw_results, f_results, 6);
  raw_rms_current[2 * index] = f_results[0] * 2.0f;
  raw_rms_voltage[2 * index] = f_results[1] * 2.0f;
  raw_avg_real_power[2 * index] = fabsf(f_results[2]) * 4.0f;
  raw_avg_apparent_power[2 * index] = f_results[0] * f_results[1] * 4.0f;
  raw_rms_current[2 * index + 1] = f_results[3] * 2.0f;
  raw_rms_voltage[2 * index + 1] = f_results[4] * 2.0f;
  raw_avg_real_power[2 * index + 1] = fabsf(f_results[5]) * 4.0f;
  raw_avg_apparent_power[2 * index + 1] = f_results[3] * f_results[4] * 4.0f;
#else
  //select BTL channel based on index
  q15_t* raw_current = index == 0 ? raw_current_AB : raw_current_CD;
  q15_t* raw_voltage = index == 0 ? raw_voltage_AB : raw_voltage_CD;
  q15_t* raw_power = index == 0 ? raw_power_AB : raw_power_CD;
